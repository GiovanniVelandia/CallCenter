...
public class Dispatcher implements Runnable {
	...
	public Dispatcher(CallCenter empleadoCallCenter, Llamada llamada, int espera) {
		this.callCenter = empleadoCallCenter;
		this.llamada = llamada;
		// LLama la duracion llamada
		duracionLlamada();
		// Llama la espera de una llamada
		duracionEspera(espera);
	}
	 ...
	 * @author GIOVANNI-PC
	 */
	public void duracionEspera(int espera) {
		this.espera = espera;
	}
	...
	public void run() {
		try {
			...
			llamada.setFechaAtencion(dtf.format(fechaInicio));
			if (empleado != null) {
				...
				llamada.setFechaAtencion(dtf.format(fechaAtencion));
				if (empleado != null) {
					llamada.setEmpleado(empleado);
					procesarLLamada(empleado);
					llamada.setEstadoLLamada("Exitoso");
